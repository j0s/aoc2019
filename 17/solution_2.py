...
def main(argv):
    ...
    outputs = []
    x, y = 0, 0
    movement_routine = 'A,B,A,C,A,B,A,C,B,C'
    # want to walk:
    # R,4,L,12,L,8,R,4,L,8,R,10,R,10,R,6,R,4,L,12,L,8,R,4,
    # |----------A----|----------B------|----------A-----|
    # R,4,R,10,L,12,R,4,L,12,L,8,R,4,L,8,R,10,R,10,R,6,
    # |----C-------|----------A-----|------B----------|
    # R,4,L,12,L,8,R,4,R,4,R,10,L,12,L,8,R,10,R,10,R,6,R,4,R,10,L,12
    # |------A--------|-----C-------|-----B-----------|------C------|
    movement_functions = [
        'R,4,L,12,L,8,R,4',
        'L,8,R,10,R,10,R,6',
    ...
    video_feed = 'n'
    for inp in [movement_routine, *movement_functions, video_feed]:
        for char in inp:
            state.inputs.append(ord(char))
        state.inputs.append(ord('\n'))
    dead = False
    while not dead and state.status is not STATE_TERMINATED:
        run_intcode(state)
        while len(state.outputs) > 0:
            char = state.outputs.pop(0)
            outputs.append(chr(char))
            if char == ord('\n'):
                y += 1
                x = 0
