...
def alignment(visited_coordinates):
    """ Output the painted coordinates as ASCII to the terminal """
    alignment = 0
    for (x, y), char in visited_coordinates.items():
        if visited_coordinates.get(coord(x-1, y), None) == '#' and \
           visited_coordinates.get(coord(x+1, y), None) == '#' and \
           visited_coordinates.get(coord(x, y-1), None) == '#' and \
           visited_coordinates.get(coord(x, y+1), None) == '#' and \
           visited_coordinates.get(coord(x, y), None) == '#':
            print(f'found coord@{x},{y}')
            alignment += x*y
    return alignment
...
def main(argv):
    ...
    outputs = []
    x, y = 0, 0
    while state.status == STATE_NOT_STARTED or len(state.outputs) > 0:
        run_intcode(state)
