...
def main(argv):
    ...
    while True:
        ...
        if status == MOVEMENT_STATUS_WALL:
            ...
            visited_coords[wall_coord] = (TILE_WALL, moves)
            direction = turn_left(direction)
            print('hit wall, turn left')
        elif status == MOVEMENT_STATUS_MOVED:
            moves += 1
            robot_coord = add_coords(robot_coord, direction)
            if robot_coord not in visited_coords:
                visited_coords[robot_coord] = (TILE_GROUND, moves)
            else:
                moves = visited_coords[robot_coord][1]
            direction = turn_right(direction)
            print('moved, turn right')
